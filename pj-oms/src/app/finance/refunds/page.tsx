    ...
    <div className="min-h-screen bg-blue-950 text-white">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        ...
        <div className="flex gap-1 mb-6 flex-wrap">
          {TABS.map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`rounded-lg px-4 py-2 text-sm font-medium capitalize ${tab === t ? "bg-blue-600" : "bg-white/10 hover:bg-white/20"}`}>
              {t}
            </button>
          ))}
        </div>
        ...
        <div className="overflow-x-auto rounded-xl border border-white/10 bg-white/5">
          <table className="w-full text-sm">
            ...
            <tbody>
              {refunds.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-white/40">No refund requests found</td></tr>
              ) : refunds.map((r) => {
                const displayAmount = r.actual_refund ?? r.approved_refund ?? r.calculated_refund;
                return (
                  <tr key={r.id} className="border-b border-white/5 hover:bg-white/5">
                    <td className="px-4 py-3">
                      <Link href={`/deals/${r.deal_id}`} className="text-blue-400 hover:underline">{r.deal_number}</Link>
                    </td>
                    <td className="px-4 py-3">{r.client_name}</td>
                    <td className="px-4 py-3 text-white/60 max-w-[200px] truncate">{r.reason}</td>
                    <td className="px-4 py-3 text-right">${Number(r.total_paid).toFixed(2)}</td>
                    <td className="px-4 py-3 text-right font-medium text-red-400">${Number(displayAmount).toFixed(2)}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[r.status] ?? ""}`}>
                        {STATUS_LABELS[r.status] ?? r.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-white/60">{new Date(r.requested_at).toLocaleDateString()}</td>
                    <td className="px-4 py-3">
      ...
      {reviewRefund && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setReviewRefund(null)}>
          <div className="w-full max-w-lg rounded-xl border border-white/10 bg-blue-950 p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">Review Refund Request</h3>
            <div className="space-y-3 text-sm mb-4">
              <div className="flex justify-between"><span className="text-white/50">Deal:</span><Link href={`/deals/${reviewRefund.deal_id}`} className="text-blue-400">{reviewRefund.deal_number}</Link></div>
              <div className="flex justify-between"><span className="text-white/50">Client:</span><span>{reviewRefund.client_name}</span></div>
              <div className="flex justify-between"><span className="text-white/50">Reason:</span><span>{reviewRefund.reason}</span></div>
              <div className="flex justify-between"><span className="text-white/50">Total Paid:</span><span>${Number(reviewRefund.total_paid).toFixed(2)}</span></div>
              <div className="flex justify-between"><span className="text-white/50">Refund %:</span><span>{reviewRefund.refund_percentage}%</span></div>
              <div className="flex justify-between"><span className="text-white/50">Calculated:</span><span>${Number(reviewRefund.calculated_refund).toFixed(2)}</span></div>
              {reviewRefund.deduction_details?.length > 0 && (
                <div>
                  ...
                  {reviewRefund.deduction_details.map((d, i) => (
                    <div key={i} className="flex justify-between ml-4 text-red-400">
                      <span>{d.description}</span><span>-${Number(d.amount).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              )}
              {reviewRefund.notes && <div><span className="text-white/50">Notes:</span> <span className="text-white/70">{reviewRefund.notes}</span></div>}
            </div>

            <div className="space-y-3 mb-4">
              <div className="flex gap-2">
                <button onClick={() => setReviewAction("approve")}
                  className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium ${reviewAction === "approve" ? "bg-green-600" : "bg-white/10"}`}>
                  Approve
                </button>
                <button onClick={() => setReviewAction("reject")}
                  className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium ${reviewAction === "reject" ? "bg-red-600" : "bg-white/10"}`}>
                  Reject
                </button>
              </div>
              ...
              {reviewAction === "approve" && (
                <div>
                  <label className="block text-sm text-white/50 mb-1">Approved Amount</label>
                  <input type="number" step="0.01" value={approvedAmount} onChange={e => setApprovedAmount(e.target.value)} className={inputClass} />
                </div>
              )}

              <div>
                <label className="block text-sm text-white/50 mb-1">Review Notes</label>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setReviewRefund(null)} className="rounded-lg bg-white/10 px-4 py-2 text-sm font-bold hover:bg-white/20">Cancel</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setProcessRefund(null)}>
          <div className="w-full max-w-md rounded-xl border border-white/10 bg-blue-950 p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3 mb-4 text-sm">
              <div className="flex justify-between"><span className="text-white/50">Client:</span><span>{processRefund.client_name}</span></div>
              <div className="flex justify-between"><span className="text-white/50">Approved Amount:</span><span className="text-red-400">${Number(processRefund.approved_refund ?? 0).toFixed(2)}</span></div>
            </div>
            <div className="space-y-3 mb-4">
              <div>
                <label className="block text-sm text-white/50 mb-1">Actual Refund Amount</label>
                <input type="number" step="0.01" value={actualRefund} onChange={e => setActualRefund(e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className="block text-sm text-white/50 mb-1">Refund Method</label>
              ...
              {refundMethod === "bank_transfer" && (
                <div>
                  <label className="block text-sm text-white/50 mb-1">Client Bank Account</label>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setProcessRefund(null)} className="rounded-lg bg-white/10 px-4 py-2 text-sm font-bold hover:bg-white/20">Cancel</button>
            </div>
          </div>
        </div>
      )}
