...
export default function StudentsPage() {
  ...
  const filterSelectClass = "rounded-lg border border-gray-300 dark:border-white/20 bg-white dark:bg-blue-900 px-4 py-3 text-gray-900 dark:text-white focus:border-blue-500 dark:focus:border-blue-400 focus:outline-none";
  const filterInputClass = "rounded-lg border border-gray-300 dark:border-white/20 bg-gray-100 dark:bg-white/10 px-3 py-2 text-sm text-gray-900 dark:text-white placeholder:text-gray-500 dark:text-white/30 focus:border-blue-500 dark:focus:border-blue-400 focus:outline-none";
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        ...
        <div className="flex flex-col gap-4 mb-8 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-white/50 mt-1 sm:text-base">{filteredStudents.length} total</p>
          </div>
          <div className="flex flex-wrap gap-2 sm:gap-3">
            <button
              onClick={handleExportCsv}
              disabled={filteredStudents.length === 0}
              className="rounded-lg border border-gray-300 dark:border-white/20 px-6 py-3 font-bold text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-white/10 disabled:opacity-50"
            >
              Export CSV
            </button>
            <Link
        ...
        {isLoading ? (
          ...
          <div className="rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 p-12 text-center">
            ...
            <p className="text-gray-500 dark:text-white/50 mb-6">Add your first student to get started.</p>
            <Link
              href="/students/new"
              className="rounded-lg bg-blue-600 px-6 py-3 font-bold text-white hover:bg-blue-700"
            >
          ...
          <>
            <div className="mb-6 flex flex-col gap-3 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 p-4 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
              <select
                value={filters.department}
                onChange={(e) => setFilters((f) => ({ ...f, department: e.target.value }))}
                className={filterSelectClass}
              >
                <option value="all" className="bg-white dark:bg-blue-900 text-gray-900 dark:text-white">All Departments</option>
              ...
              <select
                ...
                className={filterSelectClass}
              >
                <option value="all" className="bg-white dark:bg-blue-900 text-gray-900 dark:text-white">All Status</option>
              ...
              <select
                ...
                className={filterSelectClass}
              >
                <option value="all" className="bg-white dark:bg-blue-900 text-gray-900 dark:text-white">All Schools</option>
                {schools.map((sc) => (
                  <option key={sc.id} value={sc.id} className="bg-white dark:bg-blue-900 text-gray-900 dark:text-white">{sc.name}</option>
                ))}
              </select>
              <input
                type="text"
          ...
          {filteredStudents.length === 0 ? (
            <p className="py-8 text-center text-gray-500 dark:text-white/50">No students match the current filters.</p>
          ) : (
          <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-white/10">
            <table className="w-full min-w-[500px] border-collapse">
              <thead>
                <tr className="border-b border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-white/70 sm:text-sm">Name</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-white/70 sm:text-sm">School</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-white/70 sm:text-sm">Department</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-white/70 sm:text-sm">Sales</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-white/70 sm:text-sm">Enrollment Date</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-white/70 sm:text-sm">Status</th>
                </tr>
              </thead>
              <tbody>
                {filteredStudents.map((student) => (
                  <tr key={student.id} className="border-b border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 last:border-b-0">
                    <td className="px-4 py-3 text-sm font-semibold sm:text-base">
  <Link href={`/students/${student.id}`} className="hover:text-blue-700 dark:text-blue-400 hover:underline">
    {student.full_name}
  </Link>
</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-white/70 text-xs sm:text-base">{student.schools?.name ?? "—"}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-white/70 text-xs sm:text-base">{DEPT_LABELS[student.department] ?? student.department}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-white/70 text-xs sm:text-base">{student.profiles?.full_name ?? "—"}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-white/70 text-xs sm:text-base">{getDisplayEnrollmentDate(student.commissions)}</td>
