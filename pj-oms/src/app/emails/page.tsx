    ...
    <div className="min-h-screen bg-blue-950 text-white">
      ...
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        ...
        <div className="mb-8">
          ...
          <p className="mt-1 text-white/60">All sent notifications — {total} records</p>
        </div>

        {/* Filters */}
        <div className="rounded-xl border border-white/10 bg-white/5 p-4 mb-6 flex flex-wrap gap-3 items-center">
          <select
            value={filterType}
          ...
          >
            ...
            {Object.entries(EMAIL_TYPE_LABELS).map(([v, l]) => (
              <option key={v} value={v} className="bg-blue-900">{l}</option>
            ))}
          </select>
          <select
            value={filterStatus}
          ...
          >
            ...
            <option value="failed" className="bg-blue-900">Failed</option>
          </select>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  ...
                  <th className="text-left px-4 py-3 text-white/50 font-medium">Recipient</th>
                  <th className="text-left px-4 py-3 text-white/50 font-medium">Subject</th>
                  <th className="text-left px-4 py-3 text-white/50 font-medium">Sent By</th>
              ...
              <tbody>
                {logs.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-8 text-center text-white/40">No email logs found.</td>
                  </tr>
                ) : logs.map(log => (
                  <>
                    <tr key={log.id} className="border-b border-white/5 hover:bg-white/5">
                      ...
                      <td className="px-4 py-3">
                        <span className="rounded-full bg-blue-500/20 text-blue-300 px-2 py-0.5 text-xs font-medium">
                          {EMAIL_TYPE_LABELS[log.email_type ?? ""] ?? log.email_type ?? "—"}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        {log.deal_id ? (
                          <Link href={`/deals/${log.deal_id}`} className="text-blue-400 hover:underline text-xs">
                            {(log.deals as { deal_number: string | null } | null)?.deal_number ?? log.deal_id.slice(0, 8)}
                          </Link>
                        ) : <span className="text-white/30">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        <div className="text-white/80">{log.recipient_name ?? "—"}</div>
                        <div className="text-white/40 text-xs">{log.recipient_email ?? ""}</div>
                      </td>
                      <td className="px-4 py-3 text-white/70 max-w-xs truncate">{log.subject ?? "—"}</td>
                      <td className="px-4 py-3 text-white/60 text-xs">{log.sent_by ? (userNames[log.sent_by] ?? "Unknown") : "System"}</td>
                      <td className="px-4 py-3">
                        <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${log.status === "sent" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}>
                          {log.status ?? "—"}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <button onClick={() => fetchBody(log.id)} className="text-xs text-white/40 hover:text-white">
                          {expandedId === log.id ? "Hide" : "Body"}
                        </button>
                      </td>
                    </tr>
                    {expandedId === log.id && expandedBody && (
                      <tr key={`${log.id}-body`} className="border-b border-white/5 bg-white/3">
                        <td colSpan={8} className="px-4 py-3">
                          <pre className="text-xs text-white/60 whitespace-pre-wrap font-mono max-h-40 overflow-y-auto bg-black/20 rounded p-3" dangerouslySetInnerHTML={{ __html: expandedBody }} />
                        </td>
                      </tr>
                    )}
                  </>
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPage(p => p - 1)} disabled={page === 0}
                className="rounded-lg border border-white/20 px-3 py-1.5 text-sm hover:bg-white/10 disabled:opacity-30">
                ← Prev
              </button>
