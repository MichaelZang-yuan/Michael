...
const ACCREDITATION_COLORS: Record<string, string> = {
  none: "bg-gray-100 dark:bg-gray-500/20 text-gray-500 dark:text-gray-400",
  standard: "bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400",
  high_volume: "bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400",
};
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        ...
        {!isLoading && companies.length > 0 && (
          <div className="mb-6 flex flex-col gap-3 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 p-4 sm:flex-row sm:flex-wrap sm:items-center">
            ...
            <select value={filters.accreditation} onChange={e => setFilters(f => ({ ...f, accreditation: e.target.value }))} className={filterSelectClass}>
              <option value="all" className="bg-white dark:bg-blue-900">All Accreditation</option>
            ...
            />
            <button onClick={() => setFilters({ department: "all", accreditation: "all", search: "" })} className="rounded-lg border border-gray-300 dark:border-white/20 px-4 py-2 text-sm font-bold hover:bg-gray-100 dark:hover:bg-white/10">
              Clear
            </button>
          </div>
        )}
        ...
        {isLoading ? (
          ...
          <div className="rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 p-12 text-center">
            ...
            <p className="text-gray-500 dark:text-white/50 mb-6">Add your first employer or company client.</p>
          ...
          <p className="py-8 text-center text-gray-500 dark:text-white/50">No companies match the current filters.</p>
        ) : (
          <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-white/10">
            <table className="w-full min-w-[600px] border-collapse">
              <thead>
                <tr className="border-b border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-white/70">Company Name</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-white/70">Trading Name</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-white/70">Accreditation</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-white/70">Region</th>
              ...
              <tbody>
                {filtered.map((c) => (
                  <tr key={c.id} className="border-b border-gray-200 dark:border-white/10 last:border-b-0 hover:bg-gray-50 dark:hover:bg-white/5 cursor-pointer" onClick={() => router.push(`/companies/${c.id}`)}>
                    <td className="px-4 py-3 text-sm font-semibold">
                      <Link href={`/companies/${c.id}`} className="hover:text-blue-700 dark:text-blue-400 hover:underline" onClick={e => e.stopPropagation()}>{c.company_name}</Link>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-white/70">{c.trading_name ?? "—"}</td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-white/70">{c.email ?? "—"}</td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-white/70">{c.phone ?? "—"}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-bold uppercase ${ACCREDITATION_COLORS[c.accreditation_status ?? "none"] ?? "bg-gray-100 dark:bg-gray-500/20 text-gray-500 dark:text-gray-400"}`}>
                        {c.accreditation_status?.replace("_", " ") ?? "none"}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-white/70">{c.region ?? "—"}</td>
